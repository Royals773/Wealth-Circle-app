...
- [ ] **Supabase's own Auth emails (signup confirmation, password reset) have custom SMTP configured separately** (Authentication → Emails → SMTP Settings), using real production credentials — this is a genuinely different configuration from the app's own `EMAIL_SMTP_*` notification-email pipeline, easy to overlook, and every **new** Supabase project defaults to a shared testing sender with a very low rate limit that will silently block real sign-ups at any meaningful volume. Confirmed as a real gap during staging setup — see `docs/phase-9-smoke-test.md`'s SMTP note.
- [ ] Scheduler trigger (item 2) points at the production URL — only after the staging smoke test has passed, not before.
- [ ] Security headers/CSP (`next.config.ts`) sanity-checked against the real production domain by actually clicking through the app, not just loading it — a real CSP bug (missing `'unsafe-inline'` in `script-src`, breaking all client-component interactivity) was found this way during staging testing and is now fixed, but any *future* CSP change should get the same full click-through treatment, since `next build` cannot catch this class of bug at all (CSP is a runtime browser enforcement).
