...
import { rejectContributionAction, reverseContributionAction } from "@/lib/actions/contributions";
...
export function ContributionReasonDialog({
  mode,
  groupId,
  recordId,
  open,
  ...
  const action = mode === "reject" ? rejectContributionAction : reverseContributionAction;
            ...
            {mode === "reverse" ? (
              <>
                <div className="flex items-center gap-2">
                  ...
                  />
                  <Label htmlFor="create-replacement" className="font-normal">
                ...
                {createReplacement ? (
                  <div className="space-y-4 rounded-md border border-border p-3">
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div className="space-y-2">
                        <Label htmlFor="rev-amount">Correct amount</Label>
                        <Input id="rev-amount" name="amountMajorUnits" type="number" min="0.01" step="0.01" />
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor="rev-payment-method">Payment method</Label>
                        <Select
                          value={paymentMethod}
                          onValueChange={(value) => setPaymentMethod(value as typeof paymentMethod)}
                        >
                          <SelectTrigger id="rev-payment-method">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div className="space-y-2">
                        <Label htmlFor="rev-period">Correct period</Label>
                        <Input id="rev-period" name="periodDate" type="date" />
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor="rev-received">Correct date received</Label>
                        <Input id="rev-received" name="receivedAt" type="date" max={today} />
                      </div>
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="rev-reference">Bank / payment reference</Label>
                      <Input id="rev-reference" name="paymentReference" type="text" maxLength={120} />
                    </div>
