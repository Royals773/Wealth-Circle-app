 ...
 *
 * "upgrade-insecure-requests" and Strict-Transport-Security are
 * production-only: both force HTTPS for every navigation and
 * subresource, but `next dev` only ever serves plain HTTP, so applying
 * them in development upgrades localhost requests to HTTPS and the
 * browser silently fails to load the page (or its scripts), breaking
 * hydration for every client component with no build-time signal.
 */
export type SecurityHeader = { key: string; value: string };

export function buildSecurityHeaders(env: string | undefined): SecurityHeader[] {
